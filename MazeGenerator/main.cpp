...
void pollCloseCmd();
void disegnaCella(int riga, int colonna);
void drawMaze();
int tastiera();
int pollCloseCmdSDL();
...
public:
  ...
  std::vector<MazePoint> getMazeKeys()
  {
    return mazeKeys;
  }
...
private:
  ...
  void placeKeys()
  {
    ...
    while (keysPlaced < KEYS_NUMBER)
    {
      ...
      {
        mazeKeys.push_back({keyPoint.r, keyPoint.c});
...
int main()
{
  blockSize = WINDOW_SIZE / MAZE_SIZE;
  window = SDL_CreateWindow("Labirinto", SDL_WINDOWPOS_UNDEFINED,
                            SDL_WINDOWPOS_UNDEFINED, MAZE_SIZE * blockSize, MAZE_SIZE * blockSize,
                            SDL_WINDOW_SHOWN);
...
int tastiera()
{
  ...
  while (SDL_PollEvent(&evento))
    ...
    {
      char carattere = evento.key.keysym.sym;
      switch (carattere)
      {
      case 82:  // Freccia Up
      case 'w': // Su
        return 3;
        break;
      case 79:  // Freccia Right
      case 'd': // right
        return 2;
        break;
      case 81:  // Freggia Down
      case 's': // Giu
        return 1;
        break;
      case 80:  // Freccia Left
      case 'a': // left
        return 0;
        break;
      case 'p':
...
void drawMaze()
{
  ...
  for (int r = 0; r < MAZE_SIZE; r++)
  {
    ...
    {
      switch (maze.getMazeMap()[r][c])
      {
      case 1: // Blocco normale
      case 4: // Blocco già analizzato
      ...
      case 3: // Arrivo
        ...
        disegnaCella(r, c);
        break;
      case 5: // Blocco percorso (giusto)
        SDL_SetRenderDrawColor(renderer, 255, 170, 120, 255);
        disegnaCella(r, c);
        break;
      case 6: // Blocco percorso (tornato indietro)
        SDL_SetRenderDrawColor(renderer, 255, 100, 100, 255);
        disegnaCella(r, c);
        break;
      case 7: // Blocco già analizzato per autorisolvere
  ...
  // Draw keys
  int key_size = blockSize / 3;
  for (MazePoint key : maze.getMazeKeys())
  {
    SDL_SetRenderDrawColor(renderer, 200, 200, 90, 255);
    for (int m = key.c * blockSize + blockSize / 3; m < (key.c + 1) * blockSize - blockSize / 3; m++)
      for (int n = key.r * blockSize + blockSize / 3; n < (key.r + 1) * blockSize - blockSize / 3; n++)
        SDL_RenderDrawPoint(renderer, m, n);
  }
  // Draw lights
  int x, y;
  int light_size = blockSize * LIGHT_SQUARE_SIZE / 100;
  for (Light l : maze.getMazeLights())
  {
    x = l.point.c * blockSize;
    y = l.point.r * blockSize;
    switch (l.direction)
    {
    case UP:
      x += (blockSize - light_size) / 2;
      break;
    case DOWN:
      x += (blockSize - light_size) / 2;
      y += blockSize - light_size;
      break;
    case LEFT:
      y += (blockSize - light_size) / 2;
      break;
    case RIGHT:
      x += blockSize - light_size;
      y += blockSize - (blockSize - light_size) / 2;
      break;
    default:
      break;
    }
    if (l.direction != Direction::NONE)
    {
      SDL_SetRenderDrawColor(renderer, 255, 0, 0, 255);
      for (int m = x; m < x + light_size; m++)
        for (int n = y; n < y + light_size; n++)
          SDL_RenderDrawPoint(renderer, m, n);
    }
  }
  SDL_RenderPresent(renderer);
...
void disegnaCella(int riga, int colonna)
{
  for (int x = colonna * blockSize; x < (colonna + 1) * blockSize; x++)
    for (int y = riga * blockSize; y < (riga + 1) * blockSize; y++)
